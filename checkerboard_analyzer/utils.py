...
def parse_data(filename):
    ''' 
    Parses input data from an Excel spreadsheet and returns a dictionary with single drug and combination data.
    Assumes that response data has already been normalized to a value between 0.0 and 1.0.
    ...
    if dropped_count > 0:
        print(f"Warning: Dropped {dropped_count} rows with missing concentration or response values.")
    ...
    df_combo_avg = df_combo_raw.groupby(['drug1_conc', 'drug2_conc'], as_index=False)['response'].mean()
